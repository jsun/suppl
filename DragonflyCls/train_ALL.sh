...
model_archs=(dummy vgg resnet mobilenet vgg19 resnet152 densenet)
...
time python train.py --class-label ${DATA_PATH}/dragonfly_classes.txt \
                ...
                --model-outpath ./weights_species/ALL__${model_arch}__${i}.pth \
                --traindata ${DATA_PATH}/dataset_ALL/train_images \
...
echo ${model_arch}
    time python train.py --class-label ${DATA_PATH}/dragonflyg_classes.txt \
                --model-arch ${model_arch} \
                --model-outpath ./weights_genus/ALLg__${model_arch}__${i}.pth \
                --traindata ${DATA_PATH}/dataset_ALLg/train_images \
