...
def train_cv(model, output, train_dataset, valid_dataset, batch_size=1024, epochs=100):
    ...
    n_hiddens = [8, 10, 12, 14, 16, 18, 20, 22]
    activate_funcs = ['relu', 'sigmoid']
    
    n_zeros = len(dropouts) * len(n_hiddens) * len(activate_funcs)
    eval_stats = {'activate_fun': [], 'n_hidden': [], 'dropout': [],
                  'train_mse': [], 'valid_mse': []}
    ...
    for activate_func in activate_funcs:
        for n_hidden in n_hiddens:
            for dropout in dropouts:
                ...
                random.seed(int(dropout * 100 + n_hidden))
                np.random.seed(int(dropout * 100 + n_hidden))
                torch.manual_seed(int(dropout * 100 + n_hidden))
                output_ = '{}_{}_{}_{}'.format(output, activate_func, n_hidden, dropout)
                ...
                eval_stats['n_hidden'].extend([n_hidden] * n_tries)
...
def train(model, output, train_dataset, valid_dataset, batch_size, epochs, n_hidden=16, dropout=0.5, activate_func='relu'):
    ...
    for i in range(10):
        jppnet = Jppnet(model_arch=model, n_hidden_1=n_hidden, dropout=dropout, activate_func=activate_func)
    ...
    print('-------------')
    print('n_hidden: {}  dropout: {}  activate_func: {}'.format(n_hidden, dropout, activate_func))
