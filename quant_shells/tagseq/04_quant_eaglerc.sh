...
if [ ${pCountHomeolog} -eq 1 ]; then
    ...
    for seqlib in `ls  ${BAM_DIR} | awk 'BEGIN{FS="__"}{print $1}' | sort | uniq`; do
        ...
        ${BIN}/featureCounts -T 1 -s 1 -t gene -g ID -a ${GTF_A} -o ${seqlib}.counts.homeolog.txt ${seqlib}.chr*.homeolog.ref.bam
        mv ${seqlib}.counts.homeolog.* ${COUNTS_DIR}
        ...
        ${BIN}/featureCounts -T 1 -s 1 -t gene -g ID -a ${GTF_A} -o ${seqlib}.counts.specific.txt ${seqlib}.chr*.specific.ref.bam
        mv ${seqlib}.counts.specific.* ${COUNTS_DIR}
